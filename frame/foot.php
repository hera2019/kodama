<!-- Jquery Core Js -->
<script src="../style/js/jquery.js"></script>

<!-- Bootstrap Core Js -->
<script src="../style/js/bootstrap.js"></script>

<!-- Slimscroll Plugin Js -->
<script src="../style/js/jquery.slimscroll.js"></script>

<!-- Waves Effect Plugin Js -->
<script src="../style/js/waves.js"></script>

<!-- Jquery CountTo Plugin Js -->
<script src="../style/js/jquery.countTo.js"></script>

<!-- Morris Plugin Js -->
<!-- <script src="../style/js/raphael.min.js"></script> -->
<!-- <script src="../style/js/morris.js"></script> -->

<!-- ChartJs -->
<script src="../style/js/Chart.bundle.js"></script>

<!-- Flot Charts Plugin Js -->
<script src="../style/js/jquery.flot.js"></script>
<script src="../style/js/jquery.flot.resize.js"></script>
<script src="../style/js/jquery.flot.pie.js"></script>
<script src="../style/js/jquery.flot.categories.js"></script>
<script src="../style/js/jquery.flot.time.js"></script>

<!-- Sparkline Chart Plugin Js -->
<!-- <script src="../style/js/jquery.sparkline.js"></script> -->

<!-- SweetAlert Plugin Js 弹出漂亮的提示框 -->
<script src="../style/js/sweetalert.min.js"></script>

<!-- Function Js
<script src="../style/js/kodama-function.js"></script> -->

<!-- Main UI Js -->
<script src="../style/js/kodama-mainui.js"></script>

<!-- Right Bar Js -->
<script src="../style/js/kodama-rightbar.js"></script>

<!-- #/ Js
<script src="../style/js/angular.min.js"></script>
<script src="../style/js/angular-route.min.js"></script>
<script src="../style/js/kodama-app.js"></script> -->

<!-- index.html demo Js -->
<!-- <script src="../style/js/index.js"></script> -->
