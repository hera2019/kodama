...
//查询班级ID、学生人数
$sql = 'SELECT ID, studentnum FROM  class';
$statement = $connection->prepare($sql);
$statement->execute();
...
foreach($record1 as $record2)
{
    ...
    if($studentnum > 0 && $classID > 0)
    {
        ...
        $sql = "SELECT COUNT(*) from attendance LEFT JOIN user on attendance.userID = user.ID
            WHERE time1 between :starttime and :endtime
            AND user.classID = :classID";
        $statement = $connection->prepare($sql);
        ...
        if(!$bFind) //生成
        {
            $sql = 'INSERT INTO classschedule(classID, classindex, property) VALUES(:classID, :classindex, :property)';
            $statement = $connection->prepare($sql);
            if ($statement->execute([':classID' => $classID, ':classindex' => $classindex, ':property' =>$property]))
            {
            }
            else
            {
        ...
        {
            $sql = 'UPDATE classschedule SET property=:property, recordtime=:recordtime  WHERE ID=:ID';
            $statement = $connection->prepare($sql);
            if ($statement->execute([':property'=>$property, ':recordtime' => $currenttime, ':ID' => $record6->ID]))
            {
            }
            else
            {
