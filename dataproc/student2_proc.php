...
if (!empty($mod))
{
  if($mod == 'add')
  {
    ...
    if(!empty($ID))
    {
      ...
      foreach($_POST as $key => $value) {
        if($key != 'mod') { //student2 ID must set, //&& $key != 'ID'
          $sqlarray[$key] = $value;
        }
      }    
      ...
      if(!empty($sqlarray))
      {
        $classdata = new Student2_Class($connection);
        $message = $classdata->AddStudent2($ID, $sqlarray);
  ...
  else if($mod == 'query')
  {
    ...
    $data = '';
    $cls = new Student2_Class($connection);
    $message = $cls->QueryStudent2($Param, $data);
  ...
  else if($mod == 'delete')
  {
    ...
    if(!empty($Param))
    {
      ...
      if(!empty($studentIDArray)) {
        ...
        $cls = new Student2_Class($connection);
        $message = $cls->DeleteStudent2($studentIDs);
